...
import { z } from "zod";
import { createAssetSubmission, uploadPublicSubmissionDocument } from "@/lib/supabase/mahabah";
...
export async function POST(request: Request) {
  ...
  if (!result.success) return NextResponse.json({ error: "الرجاء استكمال البيانات المطلوبة بشكل صحيح", issues: result.error.flatten() }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({
      ...submission,
      message: submission.status === "draft" ? "تم حفظ الأصل العقاري كمسودة" : "تم استلام الأصل العقاري بنجاح",
    }, { status: 201 });
  } catch (error) {
